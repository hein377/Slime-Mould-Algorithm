...
class Root:
    ...
    def __init__(self, obj_func=None, lb=None, ub=None, problem_size=(1,2) , verbose=False, minimize_objFunc=False):
        """
        Parameters
        ----------
        obj_func : function
    ...
    def sort_pop_and_get_global_best_agent(self, pop=None, id_best=None):                                          # biggest fitness --> smallest fitness
        """
    ...
    def update_sorted_population_and_get_global_best_agent(self, pop=None, id_best=None, g_best=None):
        """ 
        Sort the population, update the global best slime agent if necessary, and retun the latter
        """
        pop.sort(key=lambda slime: slime.fitness, reverse=True)
        current_best = pop[id_best]
        g_best = deepcopy(current_best) if current_best.get_fitness() > g_best.get_fitness() else g_best             # Compare global best slime mould w/ this batch's best slime mould
        return g_best